mockery
=======
This is a fork of the original mockery [github.com/vektra/mockery](github.com/vektra/mockery)
It adds strongly typed mock setup functions, enabling your tests to be less verbose.
e.g.:
	Given an interface with a method X
	In  addition to `X`, the following moch has the following methods:
		- `OnX`
		- `OnAnyX`
		- `OnReturnX`
		- `OnAnyReturnX`
	Each of the methods returns the created `github.com/stretchr/testify/mock.Call`
...
These are for the original mockery:
...
`go get github.com/egag/mockery/.../`, then `$GOPATH/bin/mockery`
...
// OnReturnString creates a *mock.Call to String with no input fields conifgured with no return values: _b0
func (_m *Stringer) OnReturnString(_b0 string) *mock.Call {
	return _m.OnString().Return(_b0)
}

// OnString creates a *mock.Call to String with given fields:
func (_m *Stringer) OnString() *mock.Call {
	return _m.On("String")
}
